...
export class StandardPerformanceCalculator extends PerformanceCalculator {
  ...
  private _addParams(attributes?: DifficultyAttributes, score?: IScoreInfo): void {
    if (score) {
      ...
      this._countGreat = score.statistics.get(HitResult.Great);
      this._countOk = score.statistics.get(HitResult.Ok);
      this._countMeh = score.statistics.get(HitResult.Meh);
      this._countMiss = score.statistics.get(HitResult.Miss);
